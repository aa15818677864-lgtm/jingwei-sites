(function () {
  ...
  form.addEventListener("submit", async function (event) {
    ...
    if (!endpoint || endpoint.indexOf("REPLACE_WITH") !== -1) {
      setFormStatus("表單提交網址尚未設定，請稍後再試。", "error");
      return;
    }
    ...
    try {
      ...
      window.setTimeout(function () {
        window.location.href = "xs-thanks.html";
      }, 900);
    } catch (error) {
  ...
  function buildSubmissionPayload(currentForm) {
    ...
    append("site", siteConfig.siteCode || "liuyi-criminal-xs");
    append("language", document.documentElement.lang || "zh-HK");
    append("page_title", document.title);
    append("page_url", window.location.href);
    append("name", getFieldValue(currentForm, "name"));
    append("area_code", getFieldValue(currentForm, "contact_region"));
    append("phone", getFieldValue(currentForm, "contact_phone").replace(/\D+/g, ""));
    append("wechat", getFieldValue(currentForm, "social_contact"));
    append("inquiry_type", getFieldValue(currentForm, "case_type"));
    append("message", getFieldValue(currentForm, "summary"));
    append("source", "github-pages");
    append("user_agent", navigator.userAgent);
    append("client_ip", getFieldValue(currentForm, "client_ip"));
    append("source_page", getFieldValue(currentForm, "source_page"));
    append("stage", getFieldValue(currentForm, "stage"));
    append("property_city", getFieldValue(currentForm, "property_city"));
    append("referrer", getFieldValue(currentForm, "referrer"));
    append("landing_url", getFieldValue(currentForm, "landing_url"));
    append("utm_source", getFieldValue(currentForm, "utm_source"));
    append("utm_medium", getFieldValue(currentForm, "utm_medium"));
    append("utm_campaign", getFieldValue(currentForm, "utm_campaign"));
    append("utm_term", getFieldValue(currentForm, "utm_term"));
    append("utm_content", getFieldValue(currentForm, "utm_content"));
